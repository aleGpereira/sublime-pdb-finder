            ...
            try:
                ...
                for linenum, line in enumerate(f):
                    try:
                        ...
                        next_pdb_line = (None, "EOF")
                    for mo in patt.finditer(line):
                        # Remove the non-matched groups
                        matches = [Message(msg_type, next_pdb_line[1])
                                   for msg_type, msg in mo.groupdict().iteritems()]
...
class PdbfinderRenderer(object):
    ...
    def view(self):
        ...
        if existing_results:
            ...
            v.set_scratch(True)
            v.settings().set('pdb_results', True)
        return v
        ...
        for message_type, matches in groupby(messages, key=key_func):
            ...
            if matches:
                ...
                for idx, m in enumerate(matches, 1):
                    # Don't know the file encoding
                    msg = m['match'].msg.decode('utf8', 'ignore')
    ...
    def render_to_view(self, formatted_results):
        ...
        result_view.set_syntax_file(
            'Packages/sublime-pdb-finder/pdbfinder-results.hidden-tmLanguage')
...
class WorkerThread(threading.Thread):
    ...
    def run(self):
        ...
        pdbs = self.extractor.extract()
        rendered = list(self.renderer.format(pdbs))
...
class PdbfinderCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, open_files_only=False):
        ...
        exclude_file_patterns = [fnmatch.translate(patt)
                                 for patt in exclude_file_patterns]

        file_counter = FileScanCounter()
        extractor = PdbfinderExtractor(
            settings, filepaths, dirpaths, ignored_dirs,
        ...
        worker_thread.start()
        ThreadProgress(worker_thread, 'Finding PDBs', '', file_counter)
